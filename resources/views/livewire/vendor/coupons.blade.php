        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800 flex items-center gap-2">
                    <i class="fa-solid fa-tags text-indigo-600"></i> Store Coupons & Discounts
                </h2>
                <p class="text-sm text-gray-500 mt-1">Create exclusive promo codes for your shop products</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-md border border-gray-100 h-fit">
                <h3 class="text-lg font-semibold mb-4 text-gray-800">
                    {{ $editingId ? 'Edit Store Coupon' : 'Create Store Coupon' }}
                </h3>

                <form wire:submit.prevent="saveCoupon" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Coupon Code</label>
                        <input type="text" placeholder="e.g. SHOP10" wire:model="code"
                            class="uppercase border border-gray-300 rounded-lg p-2.5 w-full text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" placeholder="e.g. 50" wire:model="value"
                    ...
                    <div class="flex items-center gap-2 pt-2">
                        <input type="checkbox" id="v_is_active" wire:model="is_active" class="rounded text-indigo-600 cursor-pointer">
                        <label for="v_is_active" class="text-sm font-medium text-gray-700 cursor-pointer">Active</label>
                    </div>

                    <div class="flex gap-2 pt-2">
                        <button type="submit"
                            class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2.5 rounded-lg transition text-sm cursor-pointer">
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl shadow-md p-6 border border-gray-100">
                <h3 class="text-lg font-semibold mb-4 text-gray-800">Your Store Coupons</h3>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($coupons as $c)
                                <tr class="hover:bg-gray-50">
                                    <td class="py-3 px-4 font-bold text-indigo-700 uppercase">{{ $c->code }}</td>
                                    ...
                                    <td class="py-3 px-4 text-right space-x-2">
                                        <button wire:click="editCoupon({{ $c->id }})" class="text-indigo-600 hover:underline cursor-pointer text-xs">Edit</button>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center py-8 text-gray-500">No store coupons created yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
