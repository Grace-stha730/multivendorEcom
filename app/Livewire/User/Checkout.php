...
class Checkout extends Component
{
    ...
    public function mount($product = null, $quantity = 1, $variants = null): void
    {
        ...
        if ($product) {
            ...
            if ($cart?->adminCoupon) $this->adminCouponId = $cart->admin_coupon_id;
            Cart_items::where('cart_id', $cart?->id)->with(['product.firstImage', 'product.variants'])->get()
                ->each(function ($cartItem) {
                    ...
                    if ($cartItem->coupon_id) $this->vendorCoupons[$cartItem->id] = $cartItem->coupon_id;
                });
        }
        if (!$this->items) redirect()->route('user.cart')->with('error', 'Your cart is empty.');
    }
    ...
    public function updatedItems($value, $path): void
    {
        ...
        if ($field === 'quantity') $this->setQuantity($key, (int) $value);
        if (str_starts_with($field, 'selected_variants.')) { $this->validateItemStock($key); $this->persistCartItem($key); }
    }
    ...
    public function setQuantity($key, int $quantity): void
    {
        ...
        if (!$this->validateItemStock($key)) $this->items[$key]['quantity'] = $old;
        $this->persistCartItem($key);
    }
    public function increment($key): void { $this->setQuantity($key, (int)$this->items[$key]['quantity'] + 1); }
    public function decrement($key): void { $this->setQuantity($key, max(1, (int)$this->items[$key]['quantity'] - 1)); }
    private function validateItemStock($key): bool { $stock = $this->stockFor($this->items[$key]); if ($this->items[$key]['quantity'] > $stock) { $this->addError("items.$key.quantity", "Only $stock available for this selection."); return false; } return true; }
    private function discountedBasePrice(array $item): float { return app(CartPricingService::class)->discountedUnitPrice(Product::findOrFail($item['product_id'])); }
    private function stockFor(array $item): int { $structured = $item['selected_variants']['variants'] ?? []; if ($structured) return (int) collect($structured)->sum('quantity'); return (int) Product::find($item['product_id'])->stock; }
    private function unitPrice(array $item): float { return $this->discountedBasePrice($item); }
    private function lineSubtotal(array $item): float { return app(CartPricingService::class)->lineSubtotal(Product::findOrFail($item['product_id']), $item['selected_variants'] ?: [], (int) $item['quantity']); }
    private function persistCartItem($key): void { $item = $this->items[$key]; if (!$item['cart_id']) return; Cart_items::whereKey($item['cart_id'])->update(['quantity'=>$item['quantity'], 'price'=>$this->unitPrice($item), 'sub_total'=>$this->lineSubtotal($item), 'selected_variants'=>$item['selected_variants']]); }
    ...
    public function removeVendorCoupon($key): void { $this->selectVendorCoupon($key, null); }

    public function updatedAdminCouponId($couponId): void
    {
        if ($this->fromCart) Cart::where('user_id', Auth::id())->update(['admin_coupon_id' => $couponId ?: null]);
    }
    ...
    public function openVariantModal($key): void
    {
        ...
        foreach ($this->items[$key]['variants'] as $variants) foreach ($variants as $variant) {
            $this->variantQuantities[$variant['id']] = (int) ($selected[$variant['id']]['quantity'] ?? 0);
        }
    }

    public function closeVariantModal(): void { $this->variantItemKey = null; $this->variantQuantities = []; }

    public function applyVariantQuantities(): void
    {
        $key = $this->variantItemKey;
        if ($key === null || ! isset($this->items[$key])) return;
        $selected = [];
        foreach ($this->items[$key]['variants'] as $attributeName => $variants) foreach ($variants as $variant) {
            $quantity = max(0, (int) ($this->variantQuantities[$variant['id']] ?? 0));
            if ($quantity > $variant['stock']) { $this->addError("variantQuantities.{$variant['id']}", "Only {$variant['stock']} available."); return; }
            if ($quantity) $selected[] = ['variant_id'=>(int) $variant['id'], 'attribute_name'=>$attributeName, 'attribute_value'=>$variant['value'], 'quantity'=>$quantity, 'price_extra'=>(float) $variant['price_extra']];
        }
        if (! $selected) { $this->addError('variantQuantities', 'Select at least one variant.'); return; }
    ...
    private function vendorOptions(array $item) { return app(CouponService::class)->eligibleVendorCouponsForProduct(Product::find($item['product_id']))->filter(fn ($coupon) => CouponUser::where('coupon_id',$coupon->id)->where('user_id',Auth::id())->exists())->values(); }
    ...
    public function placeOrder()
    {
        ...
        $order = DB::transaction(function () use ($user, $snapshot) {
            $lines = []; $totalDiscount = 0; $subtotal = 0; $usedCouponIds = [];
            ...
            foreach (collect($lines)->groupBy(fn ($line) => $line['item']['shop_id']) as $shopId => $shopLines) { $vendorOrder = VendorOrder::create(['order_id'=>$order->id,'shop_id'=>$shopId,'subtotal'=>$shopLines->sum(fn($l)=>$l['amount']-$l['discount']),'quantity'=>$shopLines->sum(fn($l)=>$l['item']['quantity']),'status'=>'Pending']); foreach ($shopLines as $line) { $item=$line['item']; $orderItem=Order_item::create(['order_id'=>$order->id,'vendor_order_id'=>$vendorOrder->id,'product_id'=>$item['product_id'],'quantity'=>$item['quantity'],'price'=>$this->unitPrice($item),'total'=>$line['amount']-$line['discount'],'coupon_id'=>$line['coupon']?->id ?: ($line['admin_coupon'] ?? null)?->id,'coupon_discount'=>$line['discount'],'selected_variants'=>$item['selected_variants']]); foreach (array_filter([$line['coupon'] ?? null, $line['admin_coupon'] ?? null]) as $coupon) { $discount = $coupon->id === ($line['coupon']?->id ?? null) ? $this->discount($coupon,$line['amount']) : max(0,$line['discount']-$this->discount($line['coupon'] ?? $coupon,$line['amount'])); CouponRedemption::create(['coupon_id'=>$coupon->id,'user_id'=>Auth::id(),'order_id'=>$order->id,'order_item_id'=>$orderItem->id,'discount_amount'=>$discount]); if (!in_array($coupon->id, $usedCouponIds, true)) { $coupon->increment('used_count'); $usedCouponIds[] = $coupon->id; } } $product=Product::lockForUpdate()->with('variants')->find($item['product_id']); $structured=$item['selected_variants']['variants'] ?? []; if ($structured) { foreach ($structured as $selection) { $product->variants()->whereKey($selection['variant_id'])->decrement('stock', $selection['quantity']); } } else { $product->decrement('stock',$item['quantity']); } } }
            if ($this->fromCart) Cart::where('user_id', Auth::id())->get()->each->delete();
            return $order;
        });
        if ($order->payment_method === 'E-Sewa') return redirect()->route('user.payment.esewa', $order->id);
        return redirect()->route('user.order')->with('success','Order successfully placed.');
    }
    public function render() { $viewItems=[]; foreach($this->items as $key=>$item) { $item['unit_price']=$this->unitPrice($item); $item['line_subtotal']=$this->lineSubtotal($item); $item['stock']=$this->stockFor($item); $item['vendor_options']=$this->vendorOptions($item); $coupon = !empty($this->vendorCoupons[$key]) ? $item['vendor_options']->firstWhere('id', $this->vendorCoupons[$key]) : null; $item['coupon_discount']=$coupon ? $this->discount($coupon, $item['line_subtotal']) : 0; $item['subtotal']=max(0, $item['line_subtotal']-$item['coupon_discount']); $viewItems[$key]=$item; } return view('livewire.user.checkout',['viewItems'=>$viewItems,'adminOptions'=>$this->adminOptions()]); }
}
